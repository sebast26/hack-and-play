...
class AiServicesTests {
    ...
    interface CreativeWritingService {
        @SystemMessage("You are a creative writing assistant specializing in {{genre}} fiction.")
        @UserMessage("Write a {{wordCount}}-word story about {{topic}} in the {{genre}} genre")
        String writeStory(@V("genre") String genre, @V("topic") String topic, @V("wordCount") int wordCount);

        @UserMessage("Generate {{count}} creative character names for a {{setting}} story")
        List<String> generateCharacterNames(@V("setting") String setting, @V("count") int count);

        @UserMessage("Rate the creativity level of this story from 1-10: {{story}}")
